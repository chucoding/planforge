...
export async function runPlan(goal: string, opts?: PlanOpts): Promise<string> {
  ...
  try {
    ...
    const repoRootFallback = dirname(templatesRoot);
    fallback += "\n\n---\n\n" + (await loadPrompt(resolve(repoRootFallback, "packages", "core", "prompts", "append-i18n.md"))) + "\n\n" + (await loadPrompt(resolve(repoRootFallback, "packages", "core", "prompts", "append-slug.md")));
    fullPrompt = fallback + "\n\n---\n\nUser goal: " + goal;
  }
