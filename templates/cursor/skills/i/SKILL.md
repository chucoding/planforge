...
2. **Run the implement script (required)** - Execute `.cursor/skills/i/scripts/run_implement.sh` with the user's prompt as arguments. Never write final code as a substitute for script execution. The command runs `planforge implement` using `planforge.json`.
