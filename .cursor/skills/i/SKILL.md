...
1. **Summarize the conversation** - Summarize the current chat (goals, decisions, constraints, relevant files or tech) in a short paragraph and write/update a markdown file in the project's **context directory** (from `planforge.json` `contextDir`; default `.cursor/context`). Use that same path so plan/implement will read it—e.g. if contextDir is `.cursor/context`, write something like `2026-03-10-implement.md` there. Keep it short and overwrite or append as needed.
