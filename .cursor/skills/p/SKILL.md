...
2. **Run the plan script (required)** - Run **one** command only: no `cd`, no `&&` (PowerShell does not support `&&`). Terminal is already in workspace root. Use `.cursor/skills/p/scripts/run_plan.ps1 "<goal>"` on Windows or `.cursor/skills/p/scripts/run_plan.sh` on mac/Linux, or `planforge plan "<goal>"`. Never create `.plan.md` content directly in chat. The command generates `.planforge/plans/YYYY-MM-DD/MMDD-<summary>-<hash>.plan.md` using `planforge.json`.
